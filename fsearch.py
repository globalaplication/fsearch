...
def fsearch(path, search_string, type, dosdevices=False, PlayOnLinux=False, wine=False):
	...
	for path in dir_list:
		...
		select = [path+'/'+select for select in list 
					if folder_test(path+'/'+select) is 1 
				if path.find('/dosdevices/') is dosdevices if path.find('PlayOnLinux') is PlayOnLinux
					if path.find('/.wine') is wine]
...
fsearch('/home/linuxmt', 'forum', '.txt .py .dat', False, False, False)